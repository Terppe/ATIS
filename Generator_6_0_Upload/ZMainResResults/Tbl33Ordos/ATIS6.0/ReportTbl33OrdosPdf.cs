...
using Microsoft.Win32;  
         ...
         //    ReportOrdoPdf Skriptdatum:  29.01.2019  10:32    

namespace ATIS.Ui.Views.Report.ListDetails
{     
    
    public class ReportOrdoPdf : ViewModelBase
    {     
         
        private static readonly ILog Log = LogManager.GetLogger(typeof(ReportOrdoPdf));
 ...
 //    Part 1    
        ...
        public static void CreateMainPdf(int id, string use)
        {
            ...
            var ordoList = ExtCrud.GetOrdosCollectionFromOrdoIdOrderBy<Tbl33Ordo>(id).FirstOrDefault();    
        
            //Child
            var subordosList = ExtCrud.GetSubordosCollectionFromOrdoIdOrderBy<Tbl36Subordo>(id);           
             
            var expertsList = ExtCrud.GetReferenceExpertsCollectionFromOrdoIdAndRefAuthorIdIsNullAndRefSourceIdIsNullOrderBy<Tbl90Reference>(id);
            var sourcesList = ExtCrud.GetReferenceSourcesCollectionFromOrdoIdAndRefAuthorIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var authorsList = ExtCrud.GetReferenceAuthorsCollectionFromOrdoIdAndRefSourceIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var commentsList = ExtCrud.GetCommentsCollectionFromOrdoIdOrderBy<Tbl93Comment>(id);   
            ...
            { 
                    ...
                    AddOrdoHaeder(pdf, ordoList);
                    AddOrdoTaxoNomenList(pdf, ordoList);   
          
                        if (subordosList.Count != 0)
                        AddSubordosChildrenList(pdf, subordosList);      
        ...
        private static void AddOrdoHaeder(PdfDocument pdf, Tbl33Ordo tbl33OrdoList)
        {
            _page = pdf.Pages[_arrInts[6]];

            var textAusgabeAuthor = PdfHelper.AuthorViewChangeWithString(ordoList.Author, ordoList.AuthorYear);    
          
            var textAusgabeNameAuthor = ordoList.OrdoName + " " + textAusgabeAuthor;  
            ...
            _arrInts = PdfHelper.PdfTbBoldLeft("countId", _arrInts, false, CultRes.StringsRes.ReportTaxonomicId + ordoList.CountId, 0);

            _arrInts[1] += _arrInts[9] + 5; //Distance to next TextBox
        } 
          
        private static void AddOrdoTaxoNomenList(PdfDocument pdf, Tbl33Ordo tbl33OrdoList)         
          
        {
            ...
            _arrInts[1] += _arrInts[9]; //Distance to next TextBox
            //----------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("kingdomLeft", _arrInts, false, CultRes.StringsRes.Ordo + ":", 0);  
          
            _arrInts = PdfHelper.PdfTbRight("kingdomRight", _arrInts, false, ordoList.OrdoName, 0);     
            ...
            //------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMtRight("synonymRight", _arrInts, ordoList.Synonym);
            ...
            //---------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbRight("commNameGerRight", _arrInts, false, ordoList.GerName + " " + CultRes.StringsRes.ReportGerman, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameEngRight", _arrInts, false, ordoList.EngName + " " + CultRes.StringsRes.ReportEnglish, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameFraRight", _arrInts, false, ordoList.FraName + " " + CultRes.StringsRes.ReportFrench, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameSpaRight", _arrInts, false, ordoList.PorName + " " + CultRes.StringsRes.ReportSpanish, 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("currStatusRight", _arrInts, false, ordoList.Valid.ToString(), 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("recordRight", _arrInts, false, Convert.ToString(ordoList.UpdaterDate, CultureInfo.InvariantCulture), 0);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("infoRight", _arrInts, ordoList.Info);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("memoRight", _arrInts, ordoList.Memo);
        ...
        private static void AddOrdoHierarchyList(PdfDocument pdf, Tbl33Ordo tbl33OrdoList)
        {
            _page = pdf.Pages[_arrInts[6]];
        ...
        private static void AddSubordosChildrenList(PdfDocument pdf, ObservableCollection<Tbl36Subordo> subordosList)
        {
            _page = pdf.Pages[_arrInts[6]];

            _arrInts = PdfHelper.PdfTbRight("childrenSubordo", _arrInts, true, CultRes.StringsRes.ReportDirectChild, 1);
            ...
            foreach (var t in subordosList)
            {
                var t1 = t.SubordoName;
                var tAllLength = 0;
                ...
                _arrInts = PdfHelper.PdfTbMoveLeft("subordoLeft" + _z1, _arrInts, false, CultRes.StringsRes.Subordo, 0);
