...
{     
    ...
    {     
        ...
        private static readonly ILog Log = LogManager.GetLogger(typeof(ReportNamePdf));
 ...
 //    Part 1    
        ...
        public static void CreateMainPdf(int id, string use)
        {
            ...
            var nameList = ExtCrud.GetNamesCollectionFromNameIdOrderBy<Tbl78Name>(id).FirstOrDefault();    
        
            //Child
            var NULLsList = ExtCrud.GetNULLCollectionFromNameIdOrderBy<Tbl68Speciesgroup>(id);           
             
            var expertsList = ExtCrud.GetReferenceExpertsCollectionFromNameIdAndRefAuthorIdIsNullAndRefSourceIdIsNullOrderBy<Tbl90Reference>(id);
            var sourcesList = ExtCrud.GetReferenceSourcesCollectionFromNameIdAndRefAuthorIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var authorsList = ExtCrud.GetReferenceAuthorsCollectionFromNameIdAndRefSourceIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var commentsList = ExtCrud.GetCommentsCollectionFromNameIdOrderBy<Tbl93Comment>(id);   
            ...
            { 
                    ...
                    AddNameHaeder(pdf, nameList);
                    AddNameTaxoNomenList(pdf, nameList);   
          
                        if (NULLsList.Count != 0)
                        AddImagesChildrenList(pdf, NULLsList);      
        ...
        private static void AddNameHaeder(PdfDocument pdf, Tbl78Name nameList)
        {
            ...
            var textAusgabeAuthor = PdfHelper.AuthorViewChangeWithString(nameList.Author, nameList.AuthorYear);    
          
            var textAusgabeNameAuthor = nameList.NameName + " " + textAusgabeAuthor;  
            ...
            _arrInts = PdfHelper.PdfTbBoldLeft("countId", _arrInts, false, CultRes.StringsRes.ReportTaxonomicId + nameList.CountId, 0);
          ...
          
        {
            ...
            _arrInts[1] += _arrInts[9]; //Distance to next TextBox
            //----------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("kingdomLeft", _arrInts, false, CultRes.StringsRes.Name + ":", 0);  
          
            _arrInts = PdfHelper.PdfTbRight("kingdomRight", _arrInts, false, nameList.NameName, 0);     
            ...
            //------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMtRight("synonymRight", _arrInts, nameList.Synonym);
            ...
            //---------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbRight("commNameGerRight", _arrInts, false, nameList.GerName + " " + CultRes.StringsRes.ReportGerman, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameEngRight", _arrInts, false, nameList.EngName + " " + CultRes.StringsRes.ReportEnglish, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameFraRight", _arrInts, false, nameList.FraName + " " + CultRes.StringsRes.ReportFrench, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameSpaRight", _arrInts, false, nameList.PorName + " " + CultRes.StringsRes.ReportSpanish, 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("currStatusRight", _arrInts, false, nameList.Valid.ToString(), 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("recordRight", _arrInts, false, Convert.ToString(nameList.UpdaterDate, CultureInfo.InvariantCulture), 0);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("infoRight", _arrInts, nameList.Info);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("memoRight", _arrInts, nameList.Memo);
        ...
        private static void AddNameHierarchyList(PdfDocument pdf, Tbl78Name nameList)
        {
            ...
            _arrInts = PdfHelper.PdfTbMoveLeft("nameLeft", _arrInts, false, CultRes.StringsRes.Name, 0);     
          
            var txtName = nameList.NameName;        
          
            var textResult = PdfHelper.NamesAuthorsForeignNamesViewChange(txtName, nameList.Author,
                nameList.AuthorYear, nameList.GerName, nameList.EngName, nameList.FraName, nameList.PorName);

            _arrInts = PdfHelper.PdfTbMtRight("nameRight", _arrInts, textResult);

            _arrInts[1] += _arrInts[9] + 2; //Distance to next TextBox
        }      
             
        private static void AddImagesChildrenList(PdfDocument pdf, ObservableCollection<Tbl68Speciesgroup> NULLsList)
        {
            _page = pdf.Pages[_arrInts[6]];
