...
//    Tbl84SynonymsViewModel Skriptdatum:  24.05.2014  10:32    
...
{   
    ...
    public class Tbl84SynonymsViewModel : Tbl03RegnumsViewModel
    {     
 ...
 //    Part 1    
        ...
        {
            get { return _getSynonymByNameCommand ?? (_getSynonymByNameCommand = new RelayCommand(delegate { GetSynonymByNameOrId(null); })); }   
        }

        private void GetSynonymByNameOrId(object o)       
        {   
   
            Tbl84SynonymsList = null;
            Tbl84SynonymsAllList = null;
            ...
            Tbl84SynonymsList =   new ObservableCollection<Tbl84Synonym>
                                                       (from x in Tbl84SynonymsRepository.Tbl84Synonyms
                                                        where x.SynonymName.StartsWith(SearchSynonymName)
                                                        orderby x.SynonymName
                                                        select x);

            Tbl84SynonymsAllList =  new ObservableCollection<Tbl84Synonym>
                                                      (from y in Tbl84SynonymsRepository.Tbl84Synonyms
...
SynonymsView = CollectionViewSource.GetDefaultView(Tbl84SynonymsList);
        ...
        private void AddSynonym(object o)
        {
            if (Tbl84SynonymsList == null)
                Tbl84SynonymsList = new ObservableCollection<Tbl84Synonym>();
            Tbl84SynonymsList.Add(new Tbl84Synonym{ SynonymName= CultRes.StringsRes.DatasetNew });
            SynonymsView = CollectionViewSource.GetDefaultView(Tbl84SynonymsList);
            if (SynonymsView != null)
                SynonymsView.CurrentChanged += tbl84SynonymView_CurrentChanged;
            RaisePropertyChanged();
            ...
            try
            {
                ...
                if (synonym!= null)
                {
                    if (MessageBox.Show(CultRes.StringsRes.DeleteQuestion
                                        + " " +  CurrentTbl84Synonym.SynonymName, CultRes.StringsRes.DeleteQuestion1, MessageBoxButton.YesNo) !=
                        MessageBoxResult.Yes) return;
                    Tbl84SynonymsRepository.Delete(synonym);
                    Tbl84SynonymsRepository.Save();
                    MessageBox.Show(CurrentTbl84Synonym.SynonymName + " " + CultRes.StringsRes.DeleteSuccess);
                    GetSynonymByNameOrId(o); //Refresh
                }
                else
                {
                    MessageBox.Show(CultRes.StringsRes.DeleteCan + " " + CurrentTbl84Synonym.SynonymName+ " " + CultRes.StringsRes.DeleteCan1);
                }
            }
            catch (Exception ex)
            {
        ...
        private void SaveSynonym(object o)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        Tbl84SynonymsRepository.Save();
                        MessageBox.Show(CurrentTbl84Synonym.SynonymName+ " " + CultRes.StringsRes.SaveSuccess);
                        GetSynonymByNameOrId(o);  //Refresh
                    }
                }
            }
