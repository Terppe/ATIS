...
using Te.Atis.Ui.Desktop.BusinessLayer;   
         ...
         //    ReportTbl30LegiosPdf Skriptdatum:  08.11.20201817  10:32    

namespace Te.Atis.Ui.Desktop.Views.Report.PDF
{     
    
    public class ReportTbl30LegiosPdf : ViewModelBase
    {     
 ...
 //    Part 1    
        ...
        public static void CreateMainPdf(int id, int fishId, int plantId )
        {
           ...
           var reportVm = new ReportViewModel();
            reportVm.GetTbl30LegiosById(id, fishId, plantId); 

            //From Database Tbl30Legios
            _legio = _businessLayer.SingleListTbl30LegiosByLegioId(id);
            ...
            { 
                ...
                if (reportVm.Tbl33OrdosList.Count != 0)
                    doc = AddTbl33OrdosChildrenList(doc, reportVm.Tbl33OrdosList); 
        ...
        private static Document AddTbl30LegiosHaeder(Document doc)       
        {
            ...
            var author = PdfHelper.AuthorViewChangeWithoutString(_legio.Author, _legio.AuthorYear);

            table.AddCell(new PdfPCell(new Phrase(_legio.LegioName + " " + author, LargeFont)) { Colspan = 4, Border = 0 });  // 1.-4. field
            table.AddCell(new PdfPCell(new Phrase(CultRes.StringsRes.ReportTaxonomicId + " " + Convert.ToString(_legio.CountID), StandardFont)) { Colspan = 4, Border = 0 }); // 1.-4. field
            doc.Add(table);
            return doc;
        }  
          
        private static Document AddTbl30LegiosTaxoNomenList(Document doc)           
          
        {
            ...
            table.AddCell(new PdfPCell(new Phrase(CultRes.StringsRes.Legio, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4.  field
  
          
            table.AddCell(new PdfPCell { Colspan = 1, Border = 0 });  // 1.  field
            table.AddCell(new PdfPCell(new Phrase(CultRes.StringsRes.ReportSynonyms, StandardFont)) { Border = 0 });  // 2. field
            table.AddCell(new PdfPCell(new Phrase(_legio.Synonym, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4.  field
            ...
            table.AddCell(new PdfPCell(new Phrase(_legio.GerName + " " + CultRes.StringsRes.ReportGerman, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4.  field

            table.AddCell(new PdfPCell { Colspan = 2, Border = 0 });  // 1. + 2.  field
            table.AddCell(new PdfPCell(new Phrase(_legio.EngName + " " + CultRes.StringsRes.ReportEnglish, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4.  field

            table.AddCell(new PdfPCell { Colspan = 2, Border = 0 });  // 1. + 2.  field
            table.AddCell(new PdfPCell(new Phrase(_legio.FraName + " " + CultRes.StringsRes.ReportFrench, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4.  field

            table.AddCell(new PdfPCell { Colspan = 2, Border = 0 });  // 1. + 2.  field
            table.AddCell(new PdfPCell(new Phrase(_legio.PorName + " " + CultRes.StringsRes.ReportPortuguese, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4.  field
            ...
            table.AddCell(new PdfPCell(new Phrase(Convert.ToString(_legio.Valid), StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4.  field
            ...
            table.AddCell(new PdfPCell(new Phrase(Convert.ToString(_legio.UpdaterDate, CultureInfo.InvariantCulture), StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4. field

            table.AddCell(new PdfPCell { Colspan = 1, Border = 0 });  // 1.  field
            table.AddCell(new PdfPCell(new Phrase(CultRes.StringsRes.ReportInfo, StandardFont)) { Border = 0 });  // 2. field
            table.AddCell(new PdfPCell(new Phrase(_legio.Info, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4. field

            table.AddCell(new PdfPCell { Colspan = 1, Border = 0 });  // 1.  field
            table.AddCell(new PdfPCell(new Phrase(CultRes.StringsRes.ReportMemo, StandardFont)) { Border = 0 });  // 2. field
            table.AddCell(new PdfPCell(new Phrase(_legio.Memo, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4. field
        ...
        private static Document AddTbl33OrdosChildrenList(Document doc, ObservableCollection<Tbl33Ordo> tbl33OrdosList)        
        {
            var table = new PdfPTable(4)
            {
                TotalWidth = 792f, //actual width of table in points
            ...
            foreach (var t in tbl33OrdosList)   
            {
                var t1 = t.OrdoName;     
                ...
                table.AddCell(new PdfPCell(new Phrase(CultRes.StringsRes.Ordo, SmallFont)) { Border = 0 });  // 2. field
