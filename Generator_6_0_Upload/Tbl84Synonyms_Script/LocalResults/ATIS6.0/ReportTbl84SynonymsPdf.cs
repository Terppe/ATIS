...
using Microsoft.Win32;  
         ...
         //    ReportSynonymPdf Skriptdatum:  13.11.2018  10:32    

namespace ATIS.Ui.Views.Report.ListDetails
{     
    
    public class ReportSynonymPdf : ViewModelBase
    {     
         
        private static readonly ILog Log = LogManager.GetLogger(typeof(ReportSynonymPdf));
 ...
 //    Part 1    
        ...
        public static void CreateMainPdf(int id, string use)
        {
            ...
            var synonymList = ExtCrud.GetSynonymsCollectionFromSynonymIdOrderBy<Tbl84Synonym>(id).FirstOrDefault();    
        
            //Child
            var NULLsList = ExtCrud.GetNULLCollectionFromSynonymIdOrderBy<Tbl68Speciesgroup>(id);           
             
            var expertsList = ExtCrud.GetReferenceExpertsCollectionFromSynonymIdAndRefAuthorIdIsNullAndRefSourceIdIsNullOrderBy<Tbl90Reference>(id);
            var sourcesList = ExtCrud.GetReferenceSourcesCollectionFromSynonymIdAndRefAuthorIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var authorsList = ExtCrud.GetReferenceAuthorsCollectionFromSynonymIdAndRefSourceIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var commentsList = ExtCrud.GetCommentsCollectionFromSynonymIdOrderBy<Tbl93Comment>(id);   
            ...
            { 
                    ...
                    AddSynonymHaeder(pdf, synonymList);
                    AddSynonymTaxoNomenList(pdf, synonymList);   
        ...
        private static void AddSynonymHaeder(PdfDocument pdf, Tbl84Synonym tbl84SynonymList)
        {
            _page = pdf.Pages[_arrInts[6]];

            var textAusgabeAuthor = PdfHelper.AuthorViewChangeWithString(synonymList.Author, synonymList.AuthorYear);    
          
            var textAusgabeNameAuthor = synonymList.SynonymName + " " + textAusgabeAuthor;  
            ...
            _arrInts = PdfHelper.PdfTbBoldLeft("countId", _arrInts, false, CultRes.StringsRes.ReportTaxonomicId + synonymList.CountId, 0);

            _arrInts[1] += _arrInts[9] + 5; //Distance to next TextBox
        } 
          
        private static void AddSynonymTaxoNomenList(PdfDocument pdf, Tbl84Synonym tbl84SynonymList)         
          
        {
            ...
            _arrInts[1] += _arrInts[9]; //Distance to next TextBox
            //----------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("kingdomLeft", _arrInts, false, CultRes.StringsRes.Synonym + ":", 0);  
          
            _arrInts = PdfHelper.PdfTbRight("kingdomRight", _arrInts, false, synonymList.SynonymName, 0);     
            ...
            //------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMtRight("synonymRight", _arrInts, synonymList.Synonym);
            ...
            //---------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbRight("commNameGerRight", _arrInts, false, synonymList.GerName + " " + CultRes.StringsRes.ReportGerman, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameEngRight", _arrInts, false, synonymList.EngName + " " + CultRes.StringsRes.ReportEnglish, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameFraRight", _arrInts, false, synonymList.FraName + " " + CultRes.StringsRes.ReportFrench, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameSpaRight", _arrInts, false, synonymList.PorName + " " + CultRes.StringsRes.ReportSpanish, 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("currStatusRight", _arrInts, false, synonymList.Valid.ToString(), 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("recordRight", _arrInts, false, Convert.ToString(synonymList.UpdaterDate, CultureInfo.InvariantCulture), 0);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("infoRight", _arrInts, synonymList.Info);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("memoRight", _arrInts, synonymList.Memo);
        ...
        private static void AddSynonymHierarchyList(PdfDocument pdf, Tbl84Synonym tbl84SynonymList)
        {
            _page = pdf.Pages[_arrInts[6]];

            _arrInts = PdfHelper.PdfTbMoveLeft("synonymLeft", _arrInts, false, CultRes.StringsRes.Synonym, 0);     
          
            var txtName = synonymList.SynonymName;        
          
            var textResult = PdfHelper.NamesAuthorsForeignNamesViewChange(txtName, synonymList.Author,
                synonymList.AuthorYear, synonymList.GerName, synonymList.EngName, synonymList.FraName, synonymList.PorName);

            _arrInts = PdfHelper.PdfTbMtRight("synonymRight", _arrInts, textResult);
