...
using Microsoft.Win32;  
         ...
         //    ReportReferencePdf Skriptdatum:  29.11.2018  10:32    

namespace ATIS.Ui.Views.Report.ListDetails
{     
    
    public class ReportReferencePdf : ViewModelBase
    {     
         
        private static readonly ILog Log = LogManager.GetLogger(typeof(ReportReferencePdf));
 ...
 //    Part 1    
        ...
        public static void CreateMainPdf(int id, string use)
        {
            ...
            var referenceList = ExtCrud.GetReferencesCollectionFromReferenceIdOrderBy<Tbl90Reference>(id).FirstOrDefault();    
        
            //Child
            var refauthorsList = ExtCrud.GetRefAuthorsCollectionFromReferenceIdOrderBy<Tbl90RefAuthor>(id);           
             
            var expertsList = ExtCrud.GetReferenceExpertsCollectionFromReferenceIdAndRefAuthorIdIsNullAndRefSourceIdIsNullOrderBy<Tbl90Reference>(id);
            var sourcesList = ExtCrud.GetReferenceSourcesCollectionFromReferenceIdAndRefAuthorIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var authorsList = ExtCrud.GetReferenceAuthorsCollectionFromReferenceIdAndRefSourceIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var commentsList = ExtCrud.GetCommentsCollectionFromReferenceIdOrderBy<Tbl93Comment>(id);   
            ...
            { 
                    ...
                    AddReferenceHaeder(pdf, referenceList);
                    AddReferenceTaxoNomenList(pdf, referenceList);   
          
                        if (refauthorsList.Count != 0)
                        AddRefAuthorsChildrenList(pdf, refauthorsList);      
        ...
        private static void AddReferenceHaeder(PdfDocument pdf, Tbl90Reference tbl90ReferenceList)
        {
            _page = pdf.Pages[_arrInts[6]];

            var textAusgabeAuthor = PdfHelper.AuthorViewChangeWithString(referenceList.Author, referenceList.AuthorYear);    
          
            var textAusgabeNameAuthor = referenceList.ReferenceName + " " + textAusgabeAuthor;  
            ...
            _arrInts = PdfHelper.PdfTbBoldLeft("countId", _arrInts, false, CultRes.StringsRes.ReportTaxonomicId + referenceList.CountId, 0);

            _arrInts[1] += _arrInts[9] + 5; //Distance to next TextBox
        } 
          
        private static void AddReferenceTaxoNomenList(PdfDocument pdf, Tbl90Reference tbl90ReferenceList)         
          
        {
            ...
            _arrInts[1] += _arrInts[9]; //Distance to next TextBox
            //----------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("kingdomLeft", _arrInts, false, CultRes.StringsRes.Reference + ":", 0);  
          
            _arrInts = PdfHelper.PdfTbRight("kingdomRight", _arrInts, false, referenceList.ReferenceName, 0);     
            ...
            //------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMtRight("synonymRight", _arrInts, referenceList.Synonym);
            ...
            //---------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbRight("commNameGerRight", _arrInts, false, referenceList.GerName + " " + CultRes.StringsRes.ReportGerman, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameEngRight", _arrInts, false, referenceList.EngName + " " + CultRes.StringsRes.ReportEnglish, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameFraRight", _arrInts, false, referenceList.FraName + " " + CultRes.StringsRes.ReportFrench, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameSpaRight", _arrInts, false, referenceList.PorName + " " + CultRes.StringsRes.ReportSpanish, 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("currStatusRight", _arrInts, false, referenceList.Valid.ToString(), 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("recordRight", _arrInts, false, Convert.ToString(referenceList.UpdaterDate, CultureInfo.InvariantCulture), 0);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("infoRight", _arrInts, referenceList.Info);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("memoRight", _arrInts, referenceList.Memo);
        ...
        private static void AddReferenceHierarchyList(PdfDocument pdf, Tbl90Reference tbl90ReferenceList)
        {
            _page = pdf.Pages[_arrInts[6]];

            _arrInts = PdfHelper.PdfTbMoveLeft("referenceLeft", _arrInts, false, CultRes.StringsRes.Reference, 0);     
          
            var txtName = referenceList.ReferenceName;        
          
            var textResult = PdfHelper.NamesAuthorsForeignNamesViewChange(txtName, referenceList.Author,
                referenceList.AuthorYear, referenceList.GerName, referenceList.EngName, referenceList.FraName, referenceList.PorName);

            _arrInts = PdfHelper.PdfTbMtRight("referenceRight", _arrInts, textResult);

            _arrInts[1] += _arrInts[9] + 2; //Distance to next TextBox
        }      
             
        private static void AddRefAuthorsChildrenList(PdfDocument pdf, ObservableCollection<Tbl90RefAuthor> refauthorsList)
        {
            _page = pdf.Pages[_arrInts[6]];

            _arrInts = PdfHelper.PdfTbRight("childrenRefAuthor", _arrInts, true, CultRes.StringsRes.ReportDirectChild, 1);
            ...
            foreach (var t in refauthorsList)
            {
                ...
                _arrInts = PdfHelper.PdfTbMoveLeft("refauthorLeft" + _z1, _arrInts, false, CultRes.StringsRes.RefAuthor, 0);
