...
namespace ATIS.Ui.Views.Report
{
    public class ReportViewModel : ViewModelBase
    {
        ...
        {
            ReferencesCollection = _extReportBasicGet.CollReferencesByReferenceId(id);
            //direct children
            RefAuthorsCollection = _extReportBasicGet.CollRefAuthorsByReferenceIdAndHash(id);
            //------------------------------------------------------------------------------
            //Function
            var refexpertId = _extReportBasicGet.RefExpertIdFromReferencesCollectionSelect(id);
            //-----------------------------------------------------------------------------
            //ForeignKeyTable
            RefExpertsCollection = _extReportBasicGet.CollRefExpertsByRegnumIdAndHash(refexpertId);
            //------------------------------------------------------------------------------
            ExpertsCollection = _extReportBasicGet.CollExpertsByReferenceId(id);
            SourcesCollection = _extReportBasicGet.CollSourcesByReferenceId(id);
            AuthorsCollection = _extReportBasicGet.CollAuthorsByReferenceId(id);
            //------------------------------------------------------------------------------
            CommentsCollection = _extReportBasicGet.CollCommentsByReferenceId(id);
        }
        //------------------------------------------------------------------------------

        private RelayCommand _pdfReferencePrintCommand;
        public ICommand PdfReferencePrintCommand
        {
            get { return _pdfReferencePrintCommand ??= new RelayCommand(delegate { CreatePdfReferencePrint(_mainId); }); }
        }

        private static void CreatePdfReferencePrint(int id)
        {
            const string use = "print";
            ReportReferencePdf.CreateMainPdf(id, use);
        }
        private RelayCommand _pdfReferenceSaveCommand;
        public ICommand PdfReferenceSaveCommand
        {
            get { return _pdfReferenceSaveCommand ??= new RelayCommand(delegate { CreatePdfReferenceSave(_mainId); }); }
        }

        private static void CreatePdfReferenceSave(int id)
        {
            const string use = "save";
            ReportReferencePdf.CreateMainPdf(id, use);
        }
