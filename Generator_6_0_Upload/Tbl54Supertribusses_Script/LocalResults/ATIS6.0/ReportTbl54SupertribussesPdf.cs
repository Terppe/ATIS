...
using Te.Atis.Ui.Desktop.BusinessLayer;   
         ...
         //    ReportTbl54SupertribussesPdf Skriptdatum:  08.11.2018  10:32    

namespace Te.Atis.Ui.Desktop.Views.Report.PDF
{     
    
    public class ReportTbl54SupertribussesPdf : ViewModelBase
    {     
 ...
 //    Part 1    
        ...
        public static void CreateMainPdf(int id, int fishId, int plantId )
        {
           ...
           var reportVm = new ReportViewModel();
            reportVm.GetTbl54SupertribussesById(id, fishId, plantId); 

            //From Database Tbl54Supertribusses
            _supertribus = _businessLayer.SingleListTbl54SupertribussesBySupertribusId(id);
            ...
            { 
                ...
                if (reportVm.Tbl57TribussesList.Count != 0)
                    doc = AddTbl57TribussesChildrenList(doc, reportVm.Tbl57TribussesList); 
        ...
        private static Document AddTbl54SupertribussesHaeder(Document doc)       
        {
            ...
            var author = PdfHelper.AuthorViewChangeWithoutString(_supertribus.Author, _supertribus.AuthorYear);

            table.AddCell(new PdfPCell(new Phrase(_supertribus.SupertribusName + " " + author, LargeFont)) { Colspan = 4, Border = 0 });  // 1.-4. field
            table.AddCell(new PdfPCell(new Phrase(CultRes.StringsRes.ReportTaxonomicId + " " + Convert.ToString(_supertribus.CountID), StandardFont)) { Colspan = 4, Border = 0 }); // 1.-4. field
            doc.Add(table);
            return doc;
        }  
          
        private static Document AddTbl54SupertribussesTaxoNomenList(Document doc)           
          
        {
            ...
            table.AddCell(new PdfPCell(new Phrase(CultRes.StringsRes.Supertribus, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4.  field
  
          
            table.AddCell(new PdfPCell { Colspan = 1, Border = 0 });  // 1.  field
            table.AddCell(new PdfPCell(new Phrase(CultRes.StringsRes.ReportSynonyms, StandardFont)) { Border = 0 });  // 2. field
            table.AddCell(new PdfPCell(new Phrase(_supertribus.Synonym, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4.  field
            ...
            table.AddCell(new PdfPCell(new Phrase(_supertribus.GerName + " " + CultRes.StringsRes.ReportGerman, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4.  field

            table.AddCell(new PdfPCell { Colspan = 2, Border = 0 });  // 1. + 2.  field
            table.AddCell(new PdfPCell(new Phrase(_supertribus.EngName + " " + CultRes.StringsRes.ReportEnglish, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4.  field

            table.AddCell(new PdfPCell { Colspan = 2, Border = 0 });  // 1. + 2.  field
            table.AddCell(new PdfPCell(new Phrase(_supertribus.FraName + " " + CultRes.StringsRes.ReportFrench, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4.  field

            table.AddCell(new PdfPCell { Colspan = 2, Border = 0 });  // 1. + 2.  field
            table.AddCell(new PdfPCell(new Phrase(_supertribus.PorName + " " + CultRes.StringsRes.ReportPortuguese, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4.  field
            ...
            table.AddCell(new PdfPCell(new Phrase(Convert.ToString(_supertribus.Valid), StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4.  field
            ...
            table.AddCell(new PdfPCell(new Phrase(Convert.ToString(_supertribus.UpdaterDate, CultureInfo.InvariantCulture), StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4. field

            table.AddCell(new PdfPCell { Colspan = 1, Border = 0 });  // 1.  field
            table.AddCell(new PdfPCell(new Phrase(CultRes.StringsRes.ReportInfo, StandardFont)) { Border = 0 });  // 2. field
            table.AddCell(new PdfPCell(new Phrase(_supertribus.Info, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4. field

            table.AddCell(new PdfPCell { Colspan = 1, Border = 0 });  // 1.  field
            table.AddCell(new PdfPCell(new Phrase(CultRes.StringsRes.ReportMemo, StandardFont)) { Border = 0 });  // 2. field
            table.AddCell(new PdfPCell(new Phrase(_supertribus.Memo, StandardFont)) { Border = 0 });  // 3. field
            table.AddCell(new PdfPCell(new Phrase(" ")) { Border = 0 });  // 4. field
        ...
        private static Document AddTbl57TribussesChildrenList(Document doc, ObservableCollection<Tbl57Tribus> tbl57TribussesList)        
        {
            var table = new PdfPTable(4)
            {
                TotalWidth = 792f, //actual width of table in points
            ...
            foreach (var t in tbl57TribussesList)   
            {
                var t1 = t.TribusName;     
                ...
                table.AddCell(new PdfPCell(new Phrase(CultRes.StringsRes.Tribus, SmallFont)) { Border = 0 });  // 2. field
