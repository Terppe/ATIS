 ...
 //    Part 1    
        ...
        public static void CreateMainPdf(int id, int fishId, int plantId )
        {
           ...
           var reportVm = new ReportViewModel();
            reportVm.GetTbl18SuperclassesById(id, fishId, plantId); 

            //From Database Tbl18Superclasses
            _superclass = _businessLayer.SingleListTbl18SuperclassesBySuperclassId(id);
        ...
        private static Document AddTbl18SuperclassesHaeder(Document doc)       
        {
            ...
            var author = PdfHelper.AuthorViewChangeWithoutString(_superclass.Author, _superclass.AuthorYear);

            table.AddCell(new PdfPCell(new Phrase(_superclass.SuperclassName + " " + author, LargeFont)) { Colspan = 4, Border = 0 });  // 1.-4. field
