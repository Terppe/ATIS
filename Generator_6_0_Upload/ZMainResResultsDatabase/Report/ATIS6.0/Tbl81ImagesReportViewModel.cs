...
namespace ATIS.Ui.Views.Report
{
    public class ReportViewModel : ViewModelBase
    {
        ...
        {
            ImagesCollection = _extReportBasicGet.CollImagesByImageId(id);
            //direct children
            NULLCollection = _extReportBasicGet.CollNULLByImageIdAndHash(id);
            //------------------------------------------------------------------------------
            //Function
            var fispeciesId = _extReportBasicGet.FiSpeciesIdFromImagesCollectionSelect(id);
            //-----------------------------------------------------------------------------
            //ForeignKeyTable
            FiSpeciessesCollection = _extReportBasicGet.CollFiSpeciessesByRegnumIdAndHash(fispeciesId);
            //------------------------------------------------------------------------------
            ExpertsCollection = _extReportBasicGet.CollExpertsByImageId(id);
            SourcesCollection = _extReportBasicGet.CollSourcesByImageId(id);
            AuthorsCollection = _extReportBasicGet.CollAuthorsByImageId(id);
            //------------------------------------------------------------------------------
            CommentsCollection = _extReportBasicGet.CollCommentsByImageId(id);
        }
        //------------------------------------------------------------------------------

        private RelayCommand _pdfImagePrintCommand;
        public ICommand PdfImagePrintCommand
        {
            get { return _pdfImagePrintCommand ??= new RelayCommand(delegate { CreatePdfImagePrint(_mainId); }); }
        }

        private static void CreatePdfImagePrint(int id)
        {
            const string use = "print";
            ReportImagePdf.CreateMainPdf(id, use);
        }
        private RelayCommand _pdfImageSaveCommand;
        public ICommand PdfImageSaveCommand
        {
            get { return _pdfImageSaveCommand ??= new RelayCommand(delegate { CreatePdfImageSave(_mainId); }); }
        }

        private static void CreatePdfImageSave(int id)
        {
            const string use = "save";
            ReportImagePdf.CreateMainPdf(id, use);
        }
