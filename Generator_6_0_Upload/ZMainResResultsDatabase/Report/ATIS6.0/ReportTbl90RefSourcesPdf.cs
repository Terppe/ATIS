...
using Microsoft.Win32;  
         ...
         //    ReportRefSourcePdf Skriptdatum:   29.11.2018  10:32    

namespace ATIS.Ui.Views.Report.ListDetails
{     
    
    public class ReportRefSourcePdf : ViewModelBase
    {     
         
        private static readonly ILog Log = LogManager.GetLogger(typeof(ReportRefSourcePdf));
 ...
 //    Part 1    
        ...
        public static void CreateMainPdf(int id, string use)
        {
            ...
            var refSourceList = ExtGet.GetRefSourcesCollectionOrderByFromRefSourceId<Tbl90RefSource>(id).FirstOrDefault();    
        
            var NULLsList = ExtGet.GetFiSpeciessCollectionOrderByFromRefSourceId<NULL>(id);           
             
            var expertsList = ExtGet.GetReferenceExpertsCollectionOrderByFromRefSourceIdAndRefAuthorIdIsNullAndRefSourceIdIsNull<Tbl90Reference>(id);
            var sourcesList = ExtGet.GetReferenceSourcesCollectionOrderByFromRefSourceIdAndRefAuthorIdIsNullAndRefExpertIdIsNull<Tbl90Reference>(id);
            var authorsList = ExtGet.GetReferenceAuthorsCollectionOrderByFromRefSourceIdAndRefSourceIdIsNullAndRefExpertIdIsNull<Tbl90Reference>(id);
            var commentsList = ExtGet.GetCommentsCollectionOrderByFromRefSourceId<Tbl93Comment>(id);   
            ...
            { 
                ...
                using (var pdf = new PdfDocument())
                {
                    ...
                    AddRefSourceHaeder(pdf, refSourceList);
                    AddRefSourceTaxoNomenList(pdf, refSourceList);   
        ...
        private static void AddRefSourceHaeder(PdfDocument pdf, Tbl90RefSource tbl90RefSourceList)
        {
            _page = pdf.Pages[_arrInts[6]];

            var textAusgabeAuthor = PdfHelper.AuthorViewChangeWithString(refSourceList.Author, refSourceList.AuthorYear);    
          
            var textAusgabeNameAuthor = refSourceList.RefSourceName + " " + textAusgabeAuthor;  
            ...
            _arrInts = PdfHelper.PdfTbBoldLeft("countId", _arrInts, false, CultRes.StringsRes.ReportTaxonomicId + refSourceList.CountId, 0);

            _arrInts[1] += _arrInts[9] + 5; //Distance to next TextBox
        } 
          
        private static void AddRefSourceTaxoNomenList(PdfDocument pdf, Tbl90RefSource tbl90RefSourceList)         
          
        {
            ...
            _arrInts[1] += _arrInts[9]; //Distance to next TextBox
            //----------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("kingdomLeft", _arrInts, false, CultRes.StringsRes.RefSource + ":", 0);  
          
            _arrInts = PdfHelper.PdfTbRight("kingdomRight", _arrInts, false, refSourceList.RefSourceName, 0);     
            ...
            //------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMtRight("synonymRight", _arrInts, refSourceList.Synonym);
            ...
            //---------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbRight("commNameGerRight", _arrInts, false, refSourceList.GerName + " " + CultRes.StringsRes.ReportGerman, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameEngRight", _arrInts, false, refSourceList.EngName + " " + CultRes.StringsRes.ReportEnglish, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameFraRight", _arrInts, false, refSourceList.FraName + " " + CultRes.StringsRes.ReportFrench, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameSpaRight", _arrInts, false, refSourceList.PorName + " " + CultRes.StringsRes.ReportSpanish, 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("currStatusRight", _arrInts, false, refSourceList.Valid.ToString(), 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("recordRight", _arrInts, false, Convert.ToString(refSourceList.UpdaterDate, CultureInfo.InvariantCulture), 0);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("infoRight", _arrInts, refSourceList.Info);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("memoRight", _arrInts, refSourceList.Memo);
        ...
        private static void AddRefSourceHierarchyList(PdfDocument pdf, Tbl90RefSource tbl90RefSourceList)
        {
            _page = pdf.Pages[_arrInts[6]];

            _arrInts = PdfHelper.PdfTbMoveLeft("refSourceLeft", _arrInts, false, CultRes.StringsRes.RefSource, 0);     
          
            var txtName = refSourceList.RefSourceName;        
          
            var textResult = PdfHelper.NamesAuthorsForeignNamesViewChange(txtName, refSourceList.Author,
                refSourceList.AuthorYear, refSourceList.GerName, refSourceList.EngName, refSourceList.FraName, refSourceList.PorName);

            _arrInts = PdfHelper.PdfTbMtRight("refSourceRight", _arrInts, textResult);
