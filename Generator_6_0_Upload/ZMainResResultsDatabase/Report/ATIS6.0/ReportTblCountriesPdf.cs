...
using Microsoft.Win32;  
         ...
         //    ReportCountryPdf Skriptdatum:   29.11.2018 12:32      

namespace ATIS.Ui.Views.Report.ListDetails
{     
    
    public class ReportCountryPdf : ViewModelBase
    {     
         
        private static readonly ILog Log = LogManager.GetLogger(typeof(ReportCountryPdf));
 ...
 //    Part 1    
        ...
        public static void CreateMainPdf(int id, string use)
        {
            ...
            var countryList = ExtCrud.GetCountriesCollectionFromCountryIdOrderBy<TblCountry>(id).FirstOrDefault();    
        
            //Child
            var NULLsList = ExtCrud.GetCollectionFromCountryIdOrderBy<NULL>(id);           
             
            var expertsList = ExtCrud.GetReferenceExpertsCollectionFromCountryIdAndRefAuthorIdIsNullAndRefSourceIdIsNullOrderBy<Tbl90Reference>(id);
            var sourcesList = ExtCrud.GetReferenceSourcesCollectionFromCountryIdAndRefAuthorIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var authorsList = ExtCrud.GetReferenceAuthorsCollectionFromCountryIdAndRefSourceIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var commentsList = ExtCrud.GetCommentsCollectionFromCountryIdOrderBy<Tbl93Comment>(id);   
            ...
            { 
                    ...
                    AddCountryTaxoNomenList(pdf, countryList);   
        ...
        private static void AddCountryHaeder(PdfDocument pdf, TblCountry tblCountryList)
        {
            _page = pdf.Pages[_arrInts[6]];

            var textAusgabeAuthor = PdfHelper.AuthorViewChangeWithString(countryList.Author, countryList.AuthorYear);    
          
            var textAusgabeNameAuthor = countryList.CountryName + " " + textAusgabeAuthor;  
            ...
            _arrInts = PdfHelper.PdfTbBoldLeft("countId", _arrInts, false, CultRes.StringsRes.ReportTaxonomicId + countryList.CountId, 0);

            _arrInts[1] += _arrInts[9] + 5; //Distance to next TextBox
        } 
          
        private static void AddCountryTaxoNomenList(PdfDocument pdf, TblCountry tblCountryList)         
          
        {
            ...
            _arrInts[1] += _arrInts[9]; //Distance to next TextBox
            //----------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("kingdomLeft", _arrInts, false, CultRes.StringsRes.Country + ":", 0);  
          
            _arrInts = PdfHelper.PdfTbRight("kingdomRight", _arrInts, false, countryList.CountryName, 0);     
            ...
            //------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMtRight("synonymRight", _arrInts, countryList.Synonym);
            ...
            //---------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbRight("commNameGerRight", _arrInts, false, countryList.GerName + " " + CultRes.StringsRes.ReportGerman, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameEngRight", _arrInts, false, countryList.EngName + " " + CultRes.StringsRes.ReportEnglish, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameFraRight", _arrInts, false, countryList.FraName + " " + CultRes.StringsRes.ReportFrench, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameSpaRight", _arrInts, false, countryList.PorName + " " + CultRes.StringsRes.ReportSpanish, 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("currStatusRight", _arrInts, false, countryList.Valid.ToString(), 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("recordRight", _arrInts, false, Convert.ToString(countryList.UpdaterDate, CultureInfo.InvariantCulture), 0);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("infoRight", _arrInts, countryList.Info);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("memoRight", _arrInts, countryList.Memo);
        ...
        private static void AddCountryHierarchyList(PdfDocument pdf, TblCountry tblCountryList)
        {
            _page = pdf.Pages[_arrInts[6]];

            _arrInts = PdfHelper.PdfTbMoveLeft("countryLeft", _arrInts, false, CultRes.StringsRes.Country, 0);     
          
            var txtName = countryList.CountryName;        
          
            var textResult = PdfHelper.NamesAuthorsForeignNamesViewChange(txtName, countryList.Author,
                countryList.AuthorYear, countryList.GerName, countryList.EngName, countryList.FraName, countryList.PorName);

            _arrInts = PdfHelper.PdfTbMtRight("countryRight", _arrInts, textResult);
