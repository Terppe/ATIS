...
using Microsoft.Win32;  
         ...
         //    ReportRefExpertPdf Skriptdatum:  29.11.2018  10:32    

namespace ATIS.Ui.Views.Report.ListDetails
{     
    
    public class ReportRefExpertPdf : ViewModelBase
    {     
         
        private static readonly ILog Log = LogManager.GetLogger(typeof(ReportRefExpertPdf));
 ...
 //    Part 1    
        ...
        public static void CreateMainPdf(int id, string use)
        {
            ...
            var refExpertList = ExtGet.GetRefExpertsCollectionOrderByFromRefExpertId<Tbl90RefExpert>(id).FirstOrDefault();    
        
            var NULLsList = ExtGet.GetFiSpeciessCollectionOrderByFromRefExpertId<NULL>(id);           
             
            var expertsList = ExtGet.GetReferenceExpertsCollectionOrderByFromRefExpertIdAndRefAuthorIdIsNullAndRefSourceIdIsNull<Tbl90Reference>(id);
            var sourcesList = ExtGet.GetReferenceSourcesCollectionOrderByFromRefExpertIdAndRefAuthorIdIsNullAndRefExpertIdIsNull<Tbl90Reference>(id);
            var authorsList = ExtGet.GetReferenceAuthorsCollectionOrderByFromRefExpertIdAndRefSourceIdIsNullAndRefExpertIdIsNull<Tbl90Reference>(id);
            var commentsList = ExtGet.GetCommentsCollectionOrderByFromRefExpertId<Tbl93Comment>(id);   
            ...
            { 
                ...
                using (var pdf = new PdfDocument())
                {
                    ...
                    AddRefExpertHaeder(pdf, refExpertList);
                    AddRefExpertTaxoNomenList(pdf, refExpertList);   
        ...
        private static void AddRefExpertHaeder(PdfDocument pdf, Tbl90RefExpert tbl90RefExpertList)
        {
            _page = pdf.Pages[_arrInts[6]];

            var textAusgabeAuthor = PdfHelper.AuthorViewChangeWithString(refExpertList.Author, refExpertList.AuthorYear);    
          
            var textAusgabeNameAuthor = refExpertList.RefExpertName + " " + textAusgabeAuthor;  
            ...
            _arrInts = PdfHelper.PdfTbBoldLeft("countId", _arrInts, false, CultRes.StringsRes.ReportTaxonomicId + refExpertList.CountId, 0);

            _arrInts[1] += _arrInts[9] + 5; //Distance to next TextBox
        } 
          
        private static void AddRefExpertTaxoNomenList(PdfDocument pdf, Tbl90RefExpert tbl90RefExpertList)         
          
        {
            ...
            _arrInts[1] += _arrInts[9]; //Distance to next TextBox
            //----------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("kingdomLeft", _arrInts, false, CultRes.StringsRes.RefExpert + ":", 0);  
          
            _arrInts = PdfHelper.PdfTbRight("kingdomRight", _arrInts, false, refExpertList.RefExpertName, 0);     
            ...
            //------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMtRight("synonymRight", _arrInts, refExpertList.Synonym);
            ...
            //---------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbRight("commNameGerRight", _arrInts, false, refExpertList.GerName + " " + CultRes.StringsRes.ReportGerman, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameEngRight", _arrInts, false, refExpertList.EngName + " " + CultRes.StringsRes.ReportEnglish, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameFraRight", _arrInts, false, refExpertList.FraName + " " + CultRes.StringsRes.ReportFrench, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameSpaRight", _arrInts, false, refExpertList.PorName + " " + CultRes.StringsRes.ReportSpanish, 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("currStatusRight", _arrInts, false, refExpertList.Valid.ToString(), 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("recordRight", _arrInts, false, Convert.ToString(refExpertList.UpdaterDate, CultureInfo.InvariantCulture), 0);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("infoRight", _arrInts, refExpertList.Info);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("memoRight", _arrInts, refExpertList.Memo);
        ...
        private static void AddRefExpertHierarchyList(PdfDocument pdf, Tbl90RefExpert tbl90RefExpertList)
        {
            _page = pdf.Pages[_arrInts[6]];

            _arrInts = PdfHelper.PdfTbMoveLeft("refExpertLeft", _arrInts, false, CultRes.StringsRes.RefExpert, 0);     
          
            var txtName = refExpertList.RefExpertName;        
          
            var textResult = PdfHelper.NamesAuthorsForeignNamesViewChange(txtName, refExpertList.Author,
                refExpertList.AuthorYear, refExpertList.GerName, refExpertList.EngName, refExpertList.FraName, refExpertList.PorName);

            _arrInts = PdfHelper.PdfTbMtRight("refExpertRight", _arrInts, textResult);
