...
using Microsoft.Win32;  
         ...
         //    ReportSubfamilyPdf Skriptdatum:  08.11.2018  10:32    

namespace ATIS.Ui.Views.Report.ListDetails
{     
    
    public class ReportSubfamilyPdf : ViewModelBase
    {     
         
        private static readonly ILog Log = LogManager.GetLogger(typeof(ReportSubfamilyPdf));
 ...
 //    Part 1    
        ...
        public static void CreateMainPdf(int id, string use)
        {
            ...
            var subfamilyList = ExtCrud.GetSubfamiliesCollectionFromSubfamilyIdOrderBy<Tbl48Subfamily>(id).FirstOrDefault();    
        
            //Child
            var infrafamilysList = ExtCrud.GetInfrafamiliesCollectionFromSubfamilyIdOrderBy<Tbl51Infrafamily>(id);           
             
            var expertsList = ExtCrud.GetReferenceExpertsCollectionFromSubfamilyIdAndRefAuthorIdIsNullAndRefSourceIdIsNullOrderBy<Tbl90Reference>(id);
            var sourcesList = ExtCrud.GetReferenceSourcesCollectionFromSubfamilyIdAndRefAuthorIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var authorsList = ExtCrud.GetReferenceAuthorsCollectionFromSubfamilyIdAndRefSourceIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var commentsList = ExtCrud.GetCommentsCollectionFromSubfamilyIdOrderBy<Tbl93Comment>(id);   
            ...
            { 
                    ...
                    AddSubfamilyHaeder(pdf, subfamilyList);
                    AddSubfamilyTaxoNomenList(pdf, subfamilyList);   
          
                        if (infrafamilysList.Count != 0)
                        AddInfrafamilysChildrenList(pdf, infrafamilysList);      
        ...
        private static void AddSubfamilyHaeder(PdfDocument pdf, Tbl48Subfamily tbl48SubfamilyList)
        {
            _page = pdf.Pages[_arrInts[6]];

            var textAusgabeAuthor = PdfHelper.AuthorViewChangeWithString(subfamilyList.Author, subfamilyList.AuthorYear);    
          
            var textAusgabeNameAuthor = subfamilyList.SubfamilyName + " " + textAusgabeAuthor;  
            ...
            _arrInts = PdfHelper.PdfTbBoldLeft("countId", _arrInts, false, CultRes.StringsRes.ReportTaxonomicId + subfamilyList.CountId, 0);

            _arrInts[1] += _arrInts[9] + 5; //Distance to next TextBox
        } 
          
        private static void AddSubfamilyTaxoNomenList(PdfDocument pdf, Tbl48Subfamily tbl48SubfamilyList)         
          
        {
            ...
            _arrInts[1] += _arrInts[9]; //Distance to next TextBox
            //----------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("kingdomLeft", _arrInts, false, CultRes.StringsRes.Subfamily + ":", 0);  
          
            _arrInts = PdfHelper.PdfTbRight("kingdomRight", _arrInts, false, subfamilyList.SubfamilyName, 0);     
            ...
            //------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMtRight("synonymRight", _arrInts, subfamilyList.Synonym);
            ...
            //---------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbRight("commNameGerRight", _arrInts, false, subfamilyList.GerName + " " + CultRes.StringsRes.ReportGerman, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameEngRight", _arrInts, false, subfamilyList.EngName + " " + CultRes.StringsRes.ReportEnglish, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameFraRight", _arrInts, false, subfamilyList.FraName + " " + CultRes.StringsRes.ReportFrench, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameSpaRight", _arrInts, false, subfamilyList.PorName + " " + CultRes.StringsRes.ReportSpanish, 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("currStatusRight", _arrInts, false, subfamilyList.Valid.ToString(), 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("recordRight", _arrInts, false, Convert.ToString(subfamilyList.UpdaterDate, CultureInfo.InvariantCulture), 0);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("infoRight", _arrInts, subfamilyList.Info);
            ...
            _arrInts = PdfHelper.PdfTbMtRight("memoRight", _arrInts, subfamilyList.Memo);
        ...
        private static void AddSubfamilyHierarchyList(PdfDocument pdf, Tbl48Subfamily tbl48SubfamilyList)
        {
            _page = pdf.Pages[_arrInts[6]];
        ...
        private static void AddInfrafamilysChildrenList(PdfDocument pdf, ObservableCollection<Tbl51Infrafamily> infrafamilysList)
        {
            _page = pdf.Pages[_arrInts[6]];

            _arrInts = PdfHelper.PdfTbRight("childrenInfrafamily", _arrInts, true, CultRes.StringsRes.ReportDirectChild, 1);
            ...
            foreach (var t in infrafamilysList)
            {
                var t1 = t.InfrafamilyName;
                var tAllLength = 0;
                ...
                _arrInts = PdfHelper.PdfTbMoveLeft("infrafamilyLeft" + _z1, _arrInts, false, CultRes.StringsRes.Infrafamily, 0);
