...
//    Tbl51InfrafamilyViewModel Skriptdatum:  7.1.2012  10:32    
...
Tbl51InfrafamiliesList =
                 new ObservableCollection<Tbl51Infrafamily>((from x in Tbl51InfrafamiliesRepository.Tbl51Infrafamilies
                                                        where x.InfrafamilyName.StartsWith(SearchInfrafamilyName)
                                                        orderby x.InfrafamilyName
                                                        select x));

            Tbl51InfrafamiliesAllList =
                 new ObservableCollection<Tbl51Infrafamily>((from y in Tbl51InfrafamiliesRepository.Tbl51Infrafamilies
...
Tbl48SubfamiliesList = null;                  
  ...
  View = CollectionViewSource.GetDefaultView(Tbl51InfrafamiliesList);
        ...
        {
            Tbl57TribussesList =
                new ObservableCollection<Tbl57Tribus>((from  in Tbl57TribussesRepository.Tbl57Tribusses
                                                       where .TribusName.StartsWith(SearchNULLName)
                                                       orderby .TribusName
                                                       select ));
        ...
        private void DeleteNULL()
        {
            try
            {
                var = Tbl57TribussesRepository.Tbl57Tribusses.FirstOrDefault(x => x.TribusID== CurrentTbl57Tribus.TribusID);
                if (!= null)
                {
                    if (MessageBox.Show("Are you really shure to delete this "
                                        + CurrentTbl57Tribus.TribusName, "Question", MessageBoxButton.YesNo) !=
                        MessageBoxResult.Yes) return;
                    Tbl57TribussesRepository.Delete();
        ...
        private ObservableCollection<Tbl57Tribus> List;
        ...
        public void tbl54SupertribusView_CurrentChanged(object sender, EventArgs e)
        {
            RaisePropertyChanged("CurrentTbl54Supertribus");
        }
