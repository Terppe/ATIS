...
//    Tbl87GeographicsViewModel Skriptdatum:  05.06.2014  10:32    
...
{   
 ...
 //    Part 1    
        ...
        {
            get { return _getGeographicByNameCommand ?? (_getGeographicByNameCommand = new RelayCommand(delegate { GetGeographicById(null); })); }   
        }

        private void GetGeographicById(object o)       
        {   
   
            Tbl87GeographicsList = null;
            Tbl87GeographicsAllList = null;
            ...
            Tbl87GeographicsList =  new ObservableCollection<Tbl87Geographic>
                                                       (from x in Tbl87GeographicsRepository.Tbl87Geographics
                                                        where x.GeographicID.Equals(SearchGeographicId)
                                                        select x);

            Tbl87GeographicsAllList =   new ObservableCollection<Tbl87Geographic>
                                                       (from y in Tbl87GeographicsRepository.Tbl87Geographics
                                                        select y);
		    ...
		    PhoneValidatorsList = new ObservableCollection<PhoneValidator>();
			for (var i = 0; i < 267; ++i)
			{
				PhoneValidatorsList.Add(new PhoneValidator());
			}    
...
GeographicsView = CollectionViewSource.GetDefaultView(Tbl87GeographicsList);
        ...
        private void AddGeographic(object o)
        {
            if (Tbl87GeographicsList == null)
                Tbl87GeographicsList = new ObservableCollection<Tbl87Geographic>();
            Tbl87GeographicsList.Add(new Tbl87Geographic{ GeographicID= 0 });
            GeographicsView = CollectionViewSource.GetDefaultView(Tbl87GeographicsList);
            if (GeographicsView != null)
                GeographicsView.CurrentChanged += tbl87GeographicView_CurrentChanged;
            RaisePropertyChanged();
            ...
            try
            {
                ...
                if (geographic!= null)
                {
                    if (MessageBox.Show(CultRes.StringsRes.DeleteQuestion
                                        + " " +  CurrentTbl87Geographic.GeographicID, CultRes.StringsRes.DeleteQuestion1, MessageBoxButton.YesNo) !=
                        MessageBoxResult.Yes) return;
                    Tbl87GeographicsRepository.Delete(geographic);
                    Tbl87GeographicsRepository.Save();
                    MessageBox.Show(CurrentTbl87Geographic.GeographicID + " " + CultRes.StringsRes.DeleteSuccess);
                    GetGeographicById(o); //Refresh
                }
                else
                {
                    MessageBox.Show(CultRes.StringsRes.DeleteCan + " " + CurrentTbl87Geographic.GeographicID+ " " + CultRes.StringsRes.DeleteCan1);
                }
            }
            catch (Exception ex)
            {
        ...
        private void SaveGeographic(object o)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        Tbl87GeographicsRepository.Save();
                        MessageBox.Show(CurrentTbl87Geographic.GeographicID+ " " + CultRes.StringsRes.SaveSuccess);
                        GetGeographicById(o);  //Refresh
                    }
                }
            }
 ...
 //    Part 11    
		...
		{
			get { return _phoneValidatorsList; }
			set
			{
				if (_phoneValidatorsList == value) return;
				_phoneValidatorsList = value;
				RaisePropertyChanged("PhoneValidatorsList");
			}
		}
