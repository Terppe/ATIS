...
//    Tbl72PlSpeciessesViewModel Skriptdatum:  04.03.2016  10:32    
 ...
 //    Part 1    
        ...
        {   
            ...
            Tbl72PlSpeciessesList =  new ObservableCollection<Tbl72PlSpecies>
                                                       (from x in Tbl72PlSpeciessesRepository.Tbl72PlSpeciesses
                                                        where x.Tbl66Genusses.GenusName.StartsWith(SearchPlSpeciesName)
            ...
            Tbl72PlSpeciessesAllList =   new ObservableCollection<Tbl72PlSpecies>
                                                       (from y in Tbl72PlSpeciessesRepository.Tbl72PlSpeciesses
...
PlSpeciessesView = CollectionViewSource.GetDefaultView(Tbl72PlSpeciessesList);
        ...
        private void AddPlSpecies(object o)
        {
            if (Tbl72PlSpeciessesList == null)
                Tbl72PlSpeciessesList = new ObservableCollection<Tbl72PlSpecies>();
            Tbl72PlSpeciessesList.Add(new Tbl72PlSpecies{ PlSpeciesName= CultRes.StringsRes.DatasetNew });
            PlSpeciessesView = CollectionViewSource.GetDefaultView(Tbl72PlSpeciessesList);
            if (PlSpeciessesView != null)
                PlSpeciessesView.CurrentChanged += tbl72PlSpeciesView_CurrentChanged;
            RaisePropertyChanged();
            ...
            try
            {
                ...
                if (plspecies!= null)
                {
                    if (MessageBox.Show(CultRes.StringsRes.DeleteQuestion
                                        + " " +  CurrentTbl72PlSpecies.PlSpeciesName, CultRes.StringsRes.DeleteQuestion1, MessageBoxButton.YesNo) !=
                        MessageBoxResult.Yes) return;
                    Tbl72PlSpeciessesRepository.Delete(plspecies);
                    Tbl72PlSpeciessesRepository.Save();
                    MessageBox.Show(CurrentTbl72PlSpecies.PlSpeciesName + " " + CultRes.StringsRes.DeleteSuccess);
                    GetPlSpeciesByNameOrId(o); //Refresh
                }
                else
                {
                    MessageBox.Show(CultRes.StringsRes.DeleteCan + " " + CurrentTbl72PlSpecies.PlSpeciesName+ " " + CultRes.StringsRes.DeleteCan1);
                }
            }
            catch (Exception ex)
            {
        ...
        private void SavePlSpecies(object o)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        Tbl72PlSpeciessesRepository.Save();
                        MessageBox.Show(CurrentTbl72PlSpecies.PlSpeciesName+ " " + CultRes.StringsRes.SaveSuccess);
                        GetPlSpeciesByNameOrId(o);  //Refresh
                    }
                }
            }
 ...
 //    Part 5    
        ...
        {
            get { return _getImageByIDCommand ?? (_getImageByIDCommand = new RelayCommand(GetImageByID)); }
        }

        private void GetImageByID()
        {
            Tbl81ImagesList =
                new ObservableCollection<Tbl81Image>((from image in Tbl81ImagesRepository.Tbl81Images
