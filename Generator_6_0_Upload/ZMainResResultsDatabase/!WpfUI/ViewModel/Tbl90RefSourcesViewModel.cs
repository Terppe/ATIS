...
//    Tbl90RefSourcesViewModel Skriptdatum:  29.12.2011  10:32    
...
Tbl90RefSourcesList =  new ObservableCollection<Tbl90RefSource>
                                                       (from x in Tbl90RefSourcesRepository.Tbl90RefSources
            ...
            Tbl90RefSourcesAllList =  new ObservableCollection<Tbl90RefSource>
                                                       (from y in Tbl90RefSourcesRepository.Tbl90RefSources
  ...
  View = CollectionViewSource.GetDefaultView(Tbl90RefSourcesList);
        ...
        private void AddRefSource(object o)
        {
            if (Tbl90RefSourcesList == null)
                Tbl90RefSourcesList = new ObservableCollection<Tbl90RefSource>();
            Tbl90RefSourcesList.Add(new Tbl90RefSource{ RefSourceName= CultRes.StringsRes.DatasetNew });
            View = CollectionViewSource.GetDefaultView(Tbl90RefSourcesList);
            if (View != null)
                View.CurrentChanged += tbl90RefSourceView_CurrentChanged;
            RaisePropertyChanged();
            ...
            try
            {
                ...
                if (refSource!= null)
                {
                    if (MessageBox.Show(CultRes.StringsRes.DeleteQuestion
                                        + " " +  CurrentTbl90RefSource.RefSourceName, CultRes.StringsRes.DeleteQuestion1, MessageBoxButton.YesNo) !=
                        MessageBoxResult.Yes) return;
                    Tbl90RefSourcesRepository.Delete(refSource);
                    Tbl90RefSourcesRepository.Save();
                    MessageBox.Show(CurrentTbl90RefSource.RefSourceName + " " + CultRes.StringsRes.DeleteSuccess);
                    GetRefSourceByNameOrId(o); //Refresh
                }
                else
                {
                    MessageBox.Show(CultRes.StringsRes.DeleteCan + " " + CurrentTbl90RefSource.RefSourceName+ " " + CultRes.StringsRes.DeleteCan1);
                }
            }
            catch (Exception ex)
            {
        ...
        private void SaveRefSource(object o)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        Tbl90RefSourcesRepository.Save();
                        MessageBox.Show(CurrentTbl90RefSource.RefSourceName+ " " + CultRes.StringsRes.SaveSuccess);
                        GetRefSourceByNameOrId(o);  //Refresh
                    }
                }
            }
