...
//    Tbl36SubordoViewModel Skriptdatum:  7.1.2012  10:32    
...
{   
    ...
    {     
      ...
      #region "Private Data Members"
        ...
        {   
Tbl36SubordosList =
                 new ObservableCollection<Tbl36Subordo>((from x in Tbl36SubordosRepository.Tbl36Subordos
                                                        where x.SubordoName.StartsWith(SearchSubordoName)
                                                        orderby x.SubordoName
                                                        select x));

            Tbl36SubordosAllList =
                 new ObservableCollection<Tbl36Subordo>((from y in Tbl36SubordosRepository.Tbl36Subordos
  ...
  View = CollectionViewSource.GetDefaultView(Tbl36SubordosList);
        ...
        private void GetNULLByName()
        {
            Tbl42SuperfamiliesList =
                new ObservableCollection<Tbl42Superfamily>((from  in Tbl42SuperfamiliesRepository.Tbl42Superfamilies
                                                       where .SuperfamilyName.StartsWith(SearchNULLName)
                                                       orderby .SuperfamilyName
                                                       select ));
        ...
        private void DeleteNULL()
        {
            try
            {
                var = Tbl42SuperfamiliesRepository.Tbl42Superfamilies.FirstOrDefault(x => x.SuperfamilyID== CurrentTbl42Superfamily.SuperfamilyID);
                if (!= null)
                {
                    if (MessageBox.Show("Are you really shure to delete this "
                                        + CurrentTbl42Superfamily.SuperfamilyName, "Question", MessageBoxButton.YesNo) !=
                        MessageBoxResult.Yes) return;
                    Tbl42SuperfamiliesRepository.Delete();
        ...
        private ObservableCollection<Tbl42Superfamily> List;
        ...
        public void tbl39InfraordoView_CurrentChanged(object sender, EventArgs e)
        {
            RaisePropertyChanged("CurrentTbl39Infraordo");
        }
