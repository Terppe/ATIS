...
{     
    ...
    {     
        ...
        private static readonly ILog Log = LogManager.GetLogger(typeof(ReportInfrafamilyPdf));
 ...
 //    Part 1    
        ...
        public static void CreateMainPdf(int id, string use)
        {
            ...
            var infrafamilyList = ExtGet.GetInfrafamilysCollectionOrderByFromInfrafamilyId<Tbl51Infrafamily>(id).FirstOrDefault();    
        
            //Child
            var supertribussList = ExtGet.GetSupertribussesCollectionOrderByFromInfrafamilyId<Tbl54Supertribus>(id);           
             
            var expertsList = ExtGet.GetReferenceExpertsCollectionOrderByFromInfrafamilyIdAndRefAuthorIdIsNullAndRefSourceIdIsNull<Tbl90Reference>(id);
            var sourcesList = ExtGet.GetReferenceSourcesCollectionOrderByFromInfrafamilyIdAndRefAuthorIdIsNullAndRefExpertIdIsNull<Tbl90Reference>(id);
            var authorsList = ExtGet.GetReferenceAuthorsCollectionOrderByFromInfrafamilyIdAndRefSourceIdIsNullAndRefExpertIdIsNull<Tbl90Reference>(id);
            var commentsList = ExtGet.GetCommentsCollectionOrderByFromInfrafamilyId<Tbl93Comment>(id);   
            ...
            { 
                ...
                using (var pdf = new PdfDocument())
                {
                    ...
                    AddInfrafamilyHaeder(pdf, infrafamilyList);
                    AddInfrafamilyTaxoNomenList(pdf, infrafamilyList);   
          ...
          
        {
            ...
            _arrInts[1] += _arrInts[9]; //Distance to next TextBox
            //----------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("kingdomLeft", _arrInts, false, CultRes.StringsRes.Infrafamily + ":", 0);  
          
            _arrInts = PdfHelper.PdfTbRight("kingdomRight", _arrInts, false, infrafamilyList.InfrafamilyName, 0);     
