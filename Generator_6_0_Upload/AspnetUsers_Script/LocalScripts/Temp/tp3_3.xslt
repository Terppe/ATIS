...
using System.Linq; 
using ]]><xsl:value-of select="Namespace"/><![CDATA[.Domain.Models;
using ]]><xsl:value-of select="Namespace"/>.Domain.Interfaces; <![CDATA[   ]]>
<xsl:if test="Table='aspnet_Applications'">  
<![CDATA[using System;      ]]>
</xsl:if>   
<xsl:if test="Table='aspnet_Membership'">  
<![CDATA[using System;      ]]>
</xsl:if>   
<xsl:if test="Table='aspnet_Users'">  
<![CDATA[using System;      ]]>
</xsl:if>   
<![CDATA[// <!-- Repository Skriptdatum: ]]> <xsl:value-of select="DateTime"/>  <![CDATA[  -->  

namespace ]]><xsl:value-of select="Namespace"/>.Domain.Repositories <![CDATA[     {  
    public class ]]><xsl:value-of select="Table"/><![CDATA[Repository : I]]><xsl:value-of select="Table"/><![CDATA[Repository    {
        public ]]><xsl:value-of select="RegisterContext"/><![CDATA[ Entities = new ]]><xsl:value-of select="RegisterContext"/><![CDATA[();

        public IQueryable<]]><xsl:value-of select="LinqModel"/><![CDATA[> ]]><xsl:value-of select="Table"/><![CDATA[     {         
            get { return Entities.]]><xsl:value-of select="Table"/><![CDATA[; }
        }   
        //------------------------------------------------------------------------------------------     

        //  Query Methods  
        public IQueryable<]]><xsl:value-of select="LinqModel"/><![CDATA[> FindAll()    {         
            return Entities.]]><xsl:value-of select="Table"/><![CDATA[; 
        }   ]]>
<xsl:choose>
<xsl:when test="Table ='Tbl03Regnums'">
   <![CDATA[      public IQueryable<]]><xsl:value-of select="LinqModel"/><![CDATA[> FindAllSort()    {
            return from d in Entities.]]><xsl:value-of select="Table"/> <![CDATA[
...
<![CDATA[
        ...
        public void Add(]]><xsl:value-of select="EntityAbl"/><![CDATA[)    {
            Entities.]]><xsl:value-of select="Table"/><![CDATA[.AddObject(]]><xsl:value-of select="Entity"/><![CDATA[);           
        }

        public void Delete(]]><xsl:value-of select="EntityAbl"/><![CDATA[)    {
            Entities.]]><xsl:value-of select="Table"/><![CDATA[.DeleteObject(]]><xsl:value-of select="Entity"/><![CDATA[);
        } 
        //-----------------------------------------------------------------------------------------------------------
