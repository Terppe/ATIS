...
public static class Crypt
{
    ...
    public static string CalculateHash(string clearTextPassword, string saltusername)
    {
        var saltedPassword = $"{clearTextPassword}{saltusername}";
    ...
    public static string Encrypt(string clearText)
    {
        ...
        using (var encryptor = Aes.Create())
        {
          //  var pdb = new Rfc2898DeriveBytes(encryptionKey, new byte[] { 0x49, 0x76, 0x61, 0x6e, 0x20, 0x4d, 0x65, 0x64, 0x76, 0x65, 0x64, 0x65, 0x76 });
            //  var pdb = new Rfc2898DeriveBytes(encryptionKey, "Ivan Medvedev"u8.ToArray());
            var pdb = new Rfc2898DeriveBytes(encryptionKey, clearBytes, 10000, HashAlgorithmName.SHA512); 
    ...
    public static string Decrypt(string cipherText)
    {
        ...
        {
            // var pdb = new Rfc2898DeriveBytes(encryptionKey, new byte[] { 0x49, 0x76, 0x61, 0x6e, 0x20, 0x4d, 0x65, 0x64, 0x76, 0x65, 0x64, 0x65, 0x76 });
            //   var pdb = new Rfc2898DeriveBytes(encryptionKey, "elsaQ0424"u8.ToArray());
            var pdb = new Rfc2898DeriveBytes(encryptionKey, cipherBytes, 10000, HashAlgorithmName.SHA512);
            encryptor.Key = pdb.GetBytes(32);
            encryptor.IV = pdb.GetBytes(16);
            using var ms = new MemoryStream();
