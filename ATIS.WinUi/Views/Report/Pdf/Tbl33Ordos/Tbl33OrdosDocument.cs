...
namespace ATIS.WinUi.Views.Report.Pdf.Tbl33Ordos;

public class Tbl33OrdosDocument : IDocument
{
    private Tbl33OrdosPdfModel Model
    {
        get;
    }
    ...
    public Tbl33OrdosDocument(Tbl33OrdosPdfModel model)
    {
        Model = model;
    }
    ...
    private void ComposeHeaderTaxoNomen(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(column =>
            {
                ...
                column.Item().Text(text =>
                {
                    text.Span($"{Model.OrdoName}").FontSize(TextHight).SemiBold().Italic();
    ...
    {
        if (Tbl33OrdosDocumentDataSource.SubordosCollection.Count == 0)
        {
            return;
        }
    ...
    private void ComposeChildrenContent(IContainer container)
    {
        ...
        {
            foreach (var item in Tbl33OrdosDocumentDataSource.SubordosCollection)
            {
                table.ColumnsDefinition(columns =>
                {
                ...
                table.Cell().Text($"{item.SubordoName} {item.Author} {item.AuthorYear}").WrapAnywhere();
            }
        });
    }
    ...
    {
        if (Tbl33OrdosDocumentDataSource.ExpertsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl33OrdosDocumentDataSource.ExpertsCollection)
            {
                for (var i = 0; i < 4; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl33OrdosDocumentDataSource.SourcesCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl33OrdosDocumentDataSource.SourcesCollection)
            {
                for (var i = 0; i < 5; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl33OrdosDocumentDataSource.AuthorsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl33OrdosDocumentDataSource.AuthorsCollection)
            {
                for (var i = 0; i < 11; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl33OrdosDocumentDataSource.CommentsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl33OrdosDocumentDataSource.CommentsCollection)
            {
                for (var i = 0; i < 3; i++)
                {
                    if (i == 0)
