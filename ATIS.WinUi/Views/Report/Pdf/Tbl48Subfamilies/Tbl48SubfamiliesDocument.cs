...
public class Tbl48SubfamiliesDocument : IDocument
{
    private Tbl48SubfamiliesPdfModel Model
    {
        get;
    }
    ...
    public Tbl48SubfamiliesDocument(Tbl48SubfamiliesPdfModel model)
    {
        Model = model;
    }
    ...
    private void ComposeHeaderTaxoNomen(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(column =>
            {
                ...
                column.Item().Text(text =>
                {
                    text.Span($"{Model.SubfamilyName}").FontSize(TextHight).SemiBold().Italic();
    ...
    {
        if (Tbl48SubfamiliesDocumentDataSource.InfrafamiliesCollection.Count == 0)
        {
            return;
        }
    ...
    private void ComposeChildrenContent(IContainer container)
    {
        ...
        {
            foreach (var item in Tbl48SubfamiliesDocumentDataSource.InfrafamiliesCollection)
            {
                table.ColumnsDefinition(columns =>
                {
                ...
                table.Cell().Text($"{item.InfrafamilyName} {item.Author} {item.AuthorYear}").WrapAnywhere();
            }
        });
    }
    ...
    {
        if (Tbl48SubfamiliesDocumentDataSource.ExpertsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl48SubfamiliesDocumentDataSource.ExpertsCollection)
            {
                for (var i = 0; i < 4; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl48SubfamiliesDocumentDataSource.SourcesCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl48SubfamiliesDocumentDataSource.SourcesCollection)
            {
                for (var i = 0; i < 5; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl48SubfamiliesDocumentDataSource.AuthorsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl48SubfamiliesDocumentDataSource.AuthorsCollection)
            {
                for (var i = 0; i < 11; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl48SubfamiliesDocumentDataSource.CommentsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl48SubfamiliesDocumentDataSource.CommentsCollection)
            {
                for (var i = 0; i < 3; i++)
                {
                    if (i == 0)
