...
public class Tbl30LegiosDocument : IDocument
{
    private Tbl30LegiosPdfModel Model
    {
        get;
    }
    ...
    public Tbl30LegiosDocument(Tbl30LegiosPdfModel model)
    {
        Model = model;
    }
    ...
    private void ComposeHeaderTaxoNomen(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(column =>
            {
                ...
                column.Item().Text(text =>
                {
                    text.Span($"{Model.LegioName}").FontSize(TextHight).SemiBold().Italic();
    ...
    {
        if (Tbl30LegiosDocumentDataSource.OrdosCollection.Count == 0)
        {
            return;
        }
    ...
    private void ComposeChildrenContent(IContainer container)
    {
        ...
        {
            foreach (var item in Tbl30LegiosDocumentDataSource.OrdosCollection)
            {
                table.ColumnsDefinition(columns =>
                {
                ...
                table.Cell().Text($"{item.OrdoName} {item.Author} {item.AuthorYear}").WrapAnywhere();
            }
        });
    }
    ...
    {
        if (Tbl30LegiosDocumentDataSource.ExpertsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl30LegiosDocumentDataSource.ExpertsCollection)
            {
                for (var i = 0; i < 4; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl30LegiosDocumentDataSource.SourcesCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl30LegiosDocumentDataSource.SourcesCollection)
            {
                for (var i = 0; i < 5; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl30LegiosDocumentDataSource.AuthorsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl30LegiosDocumentDataSource.AuthorsCollection)
            {
                for (var i = 0; i < 11; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl30LegiosDocumentDataSource.CommentsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl30LegiosDocumentDataSource.CommentsCollection)
            {
                for (var i = 0; i < 3; i++)
                {
                    if (i == 0)
