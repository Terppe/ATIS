...
public class Tbl60SubtribussesDocument : IDocument
{
    private Tbl60SubtribussesPdfModel Model
    {
        get;
    }
    ...
    public Tbl60SubtribussesDocument(Tbl60SubtribussesPdfModel model)
    {
        Model = model;
    }
    ...
    private void ComposeHeaderTaxoNomen(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(column =>
            {
                ...
                column.Item().Text(text =>
                {
                    text.Span($"{Model.SubtribusName}, ").FontSize(TextHight).SemiBold().Italic();
    ...
    {
        if (Tbl60SubtribussesDocumentDataSource.InfratribussesCollection.Count == 0)
        {
            return;
        }
    ...
    private void ComposeChildrenContent(IContainer container)
    {
        ...
        {
            foreach (var item in Tbl60SubtribussesDocumentDataSource.InfratribussesCollection)
            {
                table.ColumnsDefinition(columns =>
                {
                ...
                table.Cell().Text($" {item.InfratribusName} {item.Author} {item.AuthorYear}").WrapAnywhere();
            }
        });
    }
    ...
    {
        if (Tbl60SubtribussesDocumentDataSource.ExpertsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl60SubtribussesDocumentDataSource.ExpertsCollection)
            {
                for (var i = 0; i < 4; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl60SubtribussesDocumentDataSource.SourcesCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl60SubtribussesDocumentDataSource.SourcesCollection)
            {
                for (var i = 0; i < 5; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl60SubtribussesDocumentDataSource.AuthorsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl60SubtribussesDocumentDataSource.AuthorsCollection)
            {
                for (var i = 0; i < 11; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl60SubtribussesDocumentDataSource.CommentsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl66GenussesDocumentDataSource.CommentsCollection)
            {
                for (var i = 0; i < 3; i++)
                {
                    if (i == 0)
