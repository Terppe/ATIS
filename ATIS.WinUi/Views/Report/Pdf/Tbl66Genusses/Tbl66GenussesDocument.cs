...
public class Tbl66GenussesDocument : IDocument
{
    private Tbl66GenussesPdfModel Model { get; }
    ...
    public Tbl66GenussesDocument(Tbl66GenussesPdfModel model)
    {
        Model = model;
    }
    ...
    private void ComposeHeaderTaxoNomen(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(column =>
            {
                ...
                column.Item().Text(text =>
                {
                    text.Span($"{Model.GenusName}, ").FontSize(TextHight).SemiBold().Italic();
    ...
    {
        if (Tbl66GenussesDocumentDataSource.FiSpeciessesCollection.Count == 0)
        {
            return;
        }
    ...
    private void ComposeChildrenContent(IContainer container)
    {
        ...
        {
            foreach (var item in Tbl66GenussesDocumentDataSource.FiSpeciessesCollection)
            {
                     table.ColumnsDefinition(columns =>
                    {
                    ...
                    table.Cell().Text($"{item.Tbl66Genusses.GenusName} {item.FiSpeciesName} {item.Subspecies} {item.Divers}" +
    ...
    {
        if (Tbl66GenussesDocumentDataSource.ExpertsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl66GenussesDocumentDataSource.ExpertsCollection)
            {
                for (var i = 0; i < 4; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl66GenussesDocumentDataSource.SourcesCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl66GenussesDocumentDataSource.SourcesCollection)
            {
                for (var i = 0; i < 5; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl66GenussesDocumentDataSource.AuthorsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl66GenussesDocumentDataSource.AuthorsCollection)
            {
                for (var i = 0; i < 11; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl66GenussesDocumentDataSource.CommentsCollection.Count == 0)
        {
            return;
        }
