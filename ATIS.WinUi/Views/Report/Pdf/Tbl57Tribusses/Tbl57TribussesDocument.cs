...
public class Tbl57TribussesDocument : IDocument
{
    private Tbl57TribussesPdfModel Model
    {
        get;
    }
    ...
    public Tbl57TribussesDocument(Tbl57TribussesPdfModel model)
    {
        Model = model;
    }
    ...
    private void ComposeHeaderTaxoNomen(IContainer container)
    {
        container.Row(row =>
        {
            row.RelativeItem().Column(column =>
            {
                ...
                column.Item().Text(text =>
                {
                    text.Span($"{Model.TribusName}").FontSize(TextHight).SemiBold().Italic();
    ...
    {
        if (Tbl57TribussesDocumentDataSource.SubtribussesCollection.Count == 0)
        {
            return;
        }
    ...
    private void ComposeChildrenContent(IContainer container)
    {
        ...
        {
            foreach (var item in Tbl57TribussesDocumentDataSource.SubtribussesCollection)
            {
                table.ColumnsDefinition(columns =>
                {
                ...
                table.Cell().Text($"{item.SubtribusName} {item.Author} {item.AuthorYear}").WrapAnywhere();
            }
        });
    }
    ...
    {
        if (Tbl57TribussesDocumentDataSource.ExpertsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl57TribussesDocumentDataSource.ExpertsCollection)
            {
                for (var i = 0; i < 4; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl57TribussesDocumentDataSource.SourcesCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl57TribussesDocumentDataSource.SourcesCollection)
            {
                for (var i = 0; i < 5; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl57TribussesDocumentDataSource.AuthorsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl57TribussesDocumentDataSource.AuthorsCollection)
            {
                for (var i = 0; i < 11; i++)
                {
                    if (i == 0)
    ...
    {
        if (Tbl57TribussesDocumentDataSource.CommentsCollection.Count == 0)
        {
            return;
        }
        ...
        container.Table(table =>
        {
            foreach (var item in Tbl57TribussesDocumentDataSource.CommentsCollection)
            {
                for (var i = 0; i < 3; i++)
                {
                    if (i == 0)
