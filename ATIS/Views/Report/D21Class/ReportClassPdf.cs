...
//    ReportClassPdf Skriptdatum:  07.01.2021  18:32    
...
namespace ATIS.Ui.Views.Report.D21Class
{
    ...
    public class ReportClassPdf : ViewModelBase
    {
...

        {
            ...
            _classSingleList = ExtCrud.GetClassSingleByClassId<Tbl21Class>(id);
            ...
            try
            {
                ...
                AddClassHaeder(pdf, _classSingleList);
                AddClassTaxoNomenList(pdf, _classSingleList, _regnumSingleList);
                ...
                if (subclasssList.Count != 0)
                    AddSubclassesChildrenList(pdf, subclasssList);
        ...
        private static void AddClassHaeder(PdfDocument pdf, Tbl21Class classList)
        {
            ...
            var textAusgabeAuthor = PdfHelper.AuthorViewChangeWithString(classList.Author, classList.AuthorYear);

            var textAusgabeNameAuthor = classList.ClassName + " " + textAusgabeAuthor;

            _arrInts = PdfHelper.PdfTbBoldLeft("className", _arrInts, true, textAusgabeNameAuthor, 2);

            _arrInts[1] += _arrInts[9]; //Distance to next TextBox

            _arrInts = PdfHelper.PdfTbBoldLeft("countId", _arrInts, false, CultRes.StringsRes.ReportTaxonomicId + classList.CountId, 0);

            _arrInts[1] += _arrInts[9] + 5; //Distance to next TextBox
        }
        private static void AddClassTaxoNomenList(PdfDocument pdf, Tbl21Class classList, Tbl03Regnum regnumList)

        {
            ...
            _arrInts = PdfHelper.PdfTbRight("rankRight", _arrInts, false, CultRes.StringsRes.Class, 0);
            //------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("synonymLeft", _arrInts, false, CultRes.StringsRes.ReportSynonyms, 0);
            //------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMtRight("synonymRight", _arrInts, classList.Synonym);
            _arrInts[1] += _arrInts[9]; //Distance to next TextBox
            //---------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("commNameLeft", _arrInts, false, CultRes.StringsRes.ReportCommonNames, 0);
            //---------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbRight("commNameGerRight", _arrInts, false, classList.GerName + " " + CultRes.StringsRes.ReportGerman, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameEngRight", _arrInts, false, classList.EngName + " " + CultRes.StringsRes.ReportEnglish, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameFraRight", _arrInts, false, classList.FraName + " " + CultRes.StringsRes.ReportFrench, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameSpaRight", _arrInts, false, classList.PorName + " " + CultRes.StringsRes.ReportSpanish, 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("currStatusRight", _arrInts, false, classList.Valid.ToString(), 0);
            _arrInts[1] += _arrInts[9] - 3; //Distance to next TextBox
            //-----------------------------------------------------------
            ...
            _arrInts = PdfHelper.PdfTbRight("recordRight", _arrInts, false, Convert.ToString(classList.UpdaterDate, CultureInfo.InvariantCulture), 0);
            _arrInts[1] += _arrInts[9] - 3; //Distance to next TextBox
            //-------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("infoLeft", _arrInts, false, CultRes.StringsRes.ReportInfo, 0);
            _arrInts = PdfHelper.PdfTbMtRight("infoRight", _arrInts, classList.Info);
            _arrInts[1] += _arrInts[9] - 3; //Distance to next TextBox
            //-------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("memoLeft", _arrInts, false, CultRes.StringsRes.ReportMemo, 0);
            _arrInts = PdfHelper.PdfTbMtRight("memoRight", _arrInts, classList.Memo);
            _arrInts[1] += _arrInts[9]; //Distance to next TextBox
        }
        ...
        private static void AddSubclassesChildrenList(PdfDocument pdf, ObservableCollection<Tbl24Subclass> subclasssList)
        {
            _page = pdf.Pages[_arrInts[6]];
