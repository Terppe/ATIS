...
//    ReportSupertribusPdf Skriptdatum:  07.01.2021  10:32    
...
namespace ATIS.Ui.Views.Report.D54Supertribus
{
    ...
    public class ReportSupertribusPdf : ViewModelBase
    {
...

        {
            ...
            _supertribusSingleList = ExtCrud.GetSupertribusSingleBySupertribusId<Tbl54Supertribus>(id);
            ...
            //Children
            var tribussList = ExtCrud.GetTribussesCollectionFromSupertribusIdOrderBy<Tbl57Tribus>(id);

            //------------------------------------------------------   
            var expertsList = ExtCrud.GetReferenceExpertsCollectionFromSupertribusIdAndRefAuthorIdIsNullAndRefSourceIdIsNullOrderBy<Tbl90Reference>(id);
            var sourcesList = ExtCrud.GetReferenceSourcesCollectionFromSupertribusIdAndRefAuthorIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var authorsList = ExtCrud.GetReferenceAuthorsCollectionFromSupertribusIdAndRefSourceIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var commentsList = ExtCrud.GetCommentsCollectionFromSupertribusIdOrderBy<Tbl93Comment>(id);
            ...
            try
            {
                ...
                AddSupertribusHaeder(pdf, _supertribusSingleList);
                AddSupertribusTaxoNomenList(pdf, _supertribusSingleList, _regnumSingleList);
        ...
        private static void AddSupertribusHaeder(PdfDocument pdf, Tbl54Supertribus supertribusList)
        {
            ...
            var textAusgabeAuthor = PdfHelper.AuthorViewChangeWithString(supertribusList.Author, supertribusList.AuthorYear);

            var textAusgabeNameAuthor = supertribusList.SupertribusName + " " + textAusgabeAuthor;

            _arrInts = PdfHelper.PdfTbBoldLeft("supertribusName", _arrInts, true, textAusgabeNameAuthor, 2);

            _arrInts[1] += _arrInts[9]; //Distance to next TextBox

            _arrInts = PdfHelper.PdfTbBoldLeft("countId", _arrInts, false, CultRes.StringsRes.ReportTaxonomicId + supertribusList.CountId, 0);

            _arrInts[1] += _arrInts[9] + 5; //Distance to next TextBox
        }

        private static void AddSupertribusTaxoNomenList(PdfDocument pdf, Tbl54Supertribus supertribusList, Tbl03Regnum regnumList)

        {
            ...
            _arrInts = PdfHelper.PdfTbRight("rankRight", _arrInts, false, CultRes.StringsRes.Supertribus, 0);
            //------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("synonymLeft", _arrInts, false, CultRes.StringsRes.ReportSynonyms, 0);
            //------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMtRight("synonymRight", _arrInts, supertribusList.Synonym);
            _arrInts[1] += _arrInts[9]; //Distance to next TextBox
            //--------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("commNameLeft", _arrInts, false, CultRes.StringsRes.ReportCommonNames, 0);
            //---------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbRight("commNameGerRight", _arrInts, false, supertribusList.GerName + " " + CultRes.StringsRes.ReportGerman, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameEngRight", _arrInts, false, supertribusList.EngName + " " + CultRes.StringsRes.ReportEnglish, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameFraRight", _arrInts, false, supertribusList.FraName + " " + CultRes.StringsRes.ReportFrench, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameSpaRight", _arrInts, false, supertribusList.PorName + " " + CultRes.StringsRes.ReportSpanish, 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("currStatusRight", _arrInts, false, supertribusList.Valid.ToString(), 0);
            _arrInts[1] += _arrInts[9] - 3; //Distance to next TextBox
            //-----------------------------------------------------------
            ...
            _arrInts = PdfHelper.PdfTbRight("recordRight", _arrInts, false, Convert.ToString(supertribusList.UpdaterDate, CultureInfo.InvariantCulture), 0);
            _arrInts[1] += _arrInts[9] - 3; //Distance to next TextBox
            //-------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("infoLeft", _arrInts, false, CultRes.StringsRes.ReportInfo, 0);
            _arrInts = PdfHelper.PdfTbMtRight("infoRight", _arrInts, supertribusList.Info);
            _arrInts[1] += _arrInts[9] - 3; //Distance to next TextBox
            //-------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("memoLeft", _arrInts, false, CultRes.StringsRes.ReportMemo, 0);
            _arrInts = PdfHelper.PdfTbMtRight("memoRight", _arrInts, supertribusList.Memo);
            _arrInts[1] += _arrInts[9]; //Distance to next TextBox
        }
        ...
        private static void AddTribussChildrenList(PdfDocument pdf, ObservableCollection<Tbl57Tribus> tribussList)
        {
            _page = pdf.Pages[_arrInts[6]];

            _arrInts = PdfHelper.PdfTbRight("childrenTribus", _arrInts, true, CultRes.StringsRes.ReportDirectChild, 1);
