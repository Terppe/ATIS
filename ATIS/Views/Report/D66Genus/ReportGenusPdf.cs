...
//    ReportGenusPdf Skriptdatum:  07.01.2021  10:32    
...
namespace ATIS.Ui.Views.Report.D66Genus
{
    ...
    public class ReportGenusPdf : ViewModelBase
    {
        ...
        private static Tbl54Supertribus _supertribusSingleList;
        private static Tbl57Tribus _tribusSingleList;
        private static Tbl60Subtribus _subtribusSingleList;
...

        {
            ...
            _genusSingleList = ExtCrud.GetGenusSingleByGenusId<Tbl66Genus>(id);
            ...
            _tribusSingleList = ExtCrud.GetTribusSingleByTribusId<Tbl57Tribus>(_subtribusSingleList.TribusId);
            _supertribusSingleList = ExtCrud.GetSupertribusSingleBySupertribusId<Tbl54Supertribus>(_tribusSingleList.SupertribusId);
            _infrafamilySingleList = ExtCrud.GetInfrafamilySingleByInfrafamilyId<Tbl51Infrafamily>(_supertribusSingleList.InfrafamilyId);
            ...
            //Children
            var fispeciessesList = ExtCrud.GetFiSpeciessesCollectionFromGenusIdOrderBy<Tbl69FiSpecies>(id);
            var plspeciessesList = ExtCrud.GetPlSpeciessesCollectionFromGenusIdOrderBy<Tbl72PlSpecies>(id);

            //------------------------------------------------------   
            var expertsList = ExtCrud.GetReferenceExpertsCollectionFromGenusIdAndRefAuthorIdIsNullAndRefSourceIdIsNullOrderBy<Tbl90Reference>(id);
            var sourcesList = ExtCrud.GetReferenceSourcesCollectionFromGenusIdAndRefAuthorIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var authorsList = ExtCrud.GetReferenceAuthorsCollectionFromGenusIdAndRefSourceIdIsNullAndRefExpertIdIsNullOrderBy<Tbl90Reference>(id);
            var commentsList = ExtCrud.GetCommentsCollectionFromGenusIdOrderBy<Tbl93Comment>(id);
            ...
            try
            {
                ...
                AddGenusHaeder(pdf, _genusSingleList);
                AddGenusTaxoNomenList(pdf, _genusSingleList, _regnumSingleList);
                ...
                if (_infrafamilySingleList != null)
                    AddInfrafamilyHierarchyList(pdf, _infrafamilySingleList);
                if (_supertribusSingleList != null)
                    AddSupertribusHierarchyList(pdf, _supertribusSingleList);
                if (_tribusSingleList != null)
                    AddTribusHierarchyList(pdf, _tribusSingleList);
                if (_subtribusSingleList != null)
                    AddSubtribusHierarchyList(pdf, _subtribusSingleList);
                if (_infratribusSingleList != null)
                    AddInfratribusHierarchyList(pdf, _infratribusSingleList);

                AddGenusHierarchyList(pdf, _genusSingleList);

                if (fispeciessesList.Count != 0)
                    AddFiSpeciessesChildrenList(pdf, fispeciessesList);

                if (plspeciessesList.Count != 0)
                    AddPlSpeciessesChildrenList(pdf, plspeciessesList);
        ...
        private static void AddGenusHaeder(PdfDocument pdf, Tbl66Genus genusList)
        {
            ...
            var textAusgabeAuthor = PdfHelper.AuthorViewChangeWithString(genusList.Author, genusList.AuthorYear);

            var textAusgabeNameAuthor = genusList.GenusName + " " + textAusgabeAuthor;

            _arrInts = PdfHelper.PdfTbBoldLeft("genusName", _arrInts, true, textAusgabeNameAuthor, 2);

            _arrInts[1] += _arrInts[9]; //Distance to next TextBox

            _arrInts = PdfHelper.PdfTbBoldLeft("countId", _arrInts, false, CultRes.StringsRes.ReportTaxonomicId + genusList.CountId, 0);

            _arrInts[1] += _arrInts[9] + 5; //Distance to next TextBox
        }

        private static void AddGenusTaxoNomenList(PdfDocument pdf, Tbl66Genus genusList, Tbl03Regnum regnumList)

        {
            ...
            _arrInts = PdfHelper.PdfTbRight("rankRight", _arrInts, false, CultRes.StringsRes.Genus, 0);
            //------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("synonymLeft", _arrInts, false, CultRes.StringsRes.ReportSynonyms, 0);
            //------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMtRight("synonymRight", _arrInts, genusList.Synonym);
            _arrInts[1] += _arrInts[9]; //Distance to next TextBox
            //--------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("commNameLeft", _arrInts, false, CultRes.StringsRes.ReportCommonNames, 0);
            //---------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbRight("commNameGerRight", _arrInts, false, genusList.GerName + " " + CultRes.StringsRes.ReportGerman, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameEngRight", _arrInts, false, genusList.EngName + " " + CultRes.StringsRes.ReportEnglish, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameFraRight", _arrInts, false, genusList.FraName + " " + CultRes.StringsRes.ReportFrench, 0);
            _arrInts = PdfHelper.PdfTbRight("commNameSpaRight", _arrInts, false, genusList.PorName + " " + CultRes.StringsRes.ReportSpanish, 0);
            ...
            _arrInts = PdfHelper.PdfTbRight("currStatusRight", _arrInts, false, genusList.Valid.ToString(), 0);
            _arrInts[1] += _arrInts[9] - 3; //Distance to next TextBox
            //-----------------------------------------------------------
            ...
            _arrInts = PdfHelper.PdfTbRight("recordRight", _arrInts, false, Convert.ToString(genusList.UpdaterDate, CultureInfo.InvariantCulture), 0);
            _arrInts[1] += _arrInts[9] - 3; //Distance to next TextBox
            //-------------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("infoLeft", _arrInts, false, CultRes.StringsRes.ReportInfo, 0);
            _arrInts = PdfHelper.PdfTbMtRight("infoRight", _arrInts, genusList.Info);
            _arrInts[1] += _arrInts[9] - 3; //Distance to next TextBox
            //-------------------------------------------------------
            _arrInts = PdfHelper.PdfTbMoveLeft("memoLeft", _arrInts, false, CultRes.StringsRes.ReportMemo, 0);
            _arrInts = PdfHelper.PdfTbMtRight("memoRight", _arrInts, genusList.Memo);
            _arrInts[1] += _arrInts[9]; //Distance to next TextBox
        }
        ...
        private static void AddSubtribusHierarchyList(PdfDocument pdf, Tbl60Subtribus subtribusList)
        {
            ...
            _arrInts = PdfHelper.PdfTbMoveLeft("subtribusLeft", _arrInts, false, CultRes.StringsRes.Subtribus, 0);

            var txtName = subtribusList.SubtribusName;

            var textResult = PdfHelper.NamesAuthorsForeignNamesViewChange(txtName, subtribusList.Author,
                subtribusList.AuthorYear, subtribusList.GerName, subtribusList.EngName, subtribusList.FraName, subtribusList.PorName);

            _arrInts = PdfHelper.PdfTbMtRight("subtribusRight", _arrInts, textResult);

            _arrInts[1] += _arrInts[9] + 2; //Distance to next TextBox
        }
        private static void AddInfratribusHierarchyList(PdfDocument pdf, Tbl63Infratribus infratribusList)
        {
            _page = pdf.Pages[_arrInts[6]];
        ...
        private static void AddGenusHierarchyList(PdfDocument pdf, Tbl66Genus genusList)
        {
            ...
            _arrInts = PdfHelper.PdfTbMoveLeft("genusLeft", _arrInts, false, CultRes.StringsRes.Genus, 0);

            var txtName = genusList.GenusName;

            var textResult = PdfHelper.NamesAuthorsForeignNamesViewChange(txtName, genusList.Author,
                genusList.AuthorYear, genusList.GerName, genusList.EngName, genusList.FraName, genusList.PorName);

            _arrInts = PdfHelper.PdfTbMtRight("genusRight", _arrInts, textResult);

            _arrInts[1] += _arrInts[9] + 2; //Distance to next TextBox
        }

        private static void AddFiSpeciessesChildrenList(PdfDocument pdf, ObservableCollection<Tbl69FiSpecies> fispeciessList)
        {
            _page = pdf.Pages[_arrInts[6]];
            ...
            foreach (var t in fispeciessList)
            {
                ...
                var t7 = t.ImportingYear;
                if (t7 != null) tAllLength += t7.Length;

                _arrInts = PdfHelper.PdfTbMoveLeft("fispeciesLeft" + _z1, _arrInts, false, CultRes.StringsRes.FiSpecies, 0);
        ...
        private static void AddPlSpeciessesChildrenList(PdfDocument pdf, ObservableCollection<Tbl72PlSpecies> plspeciessList)
        {
            _page = pdf.Pages[_arrInts[6]];

            _arrInts = PdfHelper.PdfTbRight("childrenPlSpecies", _arrInts, true, CultRes.StringsRes.ReportDirectChild, 1);
            ...
            foreach (var t in plspeciessList)
            {
                ...
                _arrInts = PdfHelper.PdfTbMoveLeft("plspeciesLeft" + _z1, _arrInts, false, CultRes.StringsRes.PlSpecies, 0);
